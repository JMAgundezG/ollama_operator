"""Asynchronous Ollama Operator client.
...
from ._model_manager import AsyncModelManager, ProgressCallback
from .types import (
    ChatResponse,
    EmbedResponse,
    GenerateResponse,
...
class AsyncOllamaOperator:
    """Asynchronous client that proxies Ollama API requests with automatic
    model management.
    ...
    def __init__(
        ...
        *,
        ...
        self._http = AsyncHTTP(base_url=base_url)
        self._models = AsyncModelManager(self._http)
    ...
    async def __aenter__(self) -> AsyncOllamaOperator:
        return self
    ...
    async def _ensure_model(self, model: str) -> None:
        if self.auto_pull:
            await self._models.ensure(model, on_progress=self.on_pull_progress)
    ...
    async def generate(
        ...
        *,
        ...
        data = await self._http.post("/api/generate", payload)
        return GenerateResponse.from_dict(data)

    async def _generate_stream(
        self, payload: dict[str, Any]
    ) -> AsyncIterator[GenerateResponse]:
        async for chunk in self._http.post_stream("/api/generate", payload):
    ...
    async def _chat_stream(
        ...
        async for chunk in self._http.post_stream("/api/chat", payload):
    ...
    async def embed(
        ...
        *,
        ...
        data = await self._http.post("/api/embed", payload)
    ...
    async def show(self, model: str, *, verbose: bool = False) -> ModelShowResponse:
        """POST /api/show - Show model information."""
        payload: dict[str, Any] = {"model": model}
        if verbose:
            payload["verbose"] = True
        data = await self._http.post("/api/show", payload)
    ...
    async def delete(self, model: str) -> None:
        ...
        await self._http.delete("/api/delete", {"model": model})
        await self._models.refresh()
    ...
    async def push(
        ...
        *,
        ...
        if insecure:
            payload["insecure"] = True
        async for chunk in self._http.post_stream("/api/push", payload):
            if on_progress is not None:
                on_progress(PullProgress.from_dict(chunk))
    ...
    async def create(
        ...
        *,
        ...
        if stream:
            async for chunk in self._http.post_stream("/api/create", payload):
                if on_progress is not None:
                    on_progress(PullProgress.from_dict(chunk))
        else:
            await self._http.post("/api/create", payload)
        await self._models.refresh()
    ...
    async def list_running(self) -> list[RunningModel]:
        ...
        data = await self._http.get("/api/ps")
    ...
    async def version(self) -> VersionResponse:
        """GET /api/version"""
        data = await self._http.get("/api/version")
